...
import { ENDPOINT, testIPs, expectedResponses } from "../../utils/apiConstants";
...
test.describe("API Testing - IP Insights", () => {
  ...
  test("Verify correct IP response", async ({ request }) => {
    const response = await request.get(`${ENDPOINT}?ip=${testIPs.valid}`);
    expect(response.status()).toBe(200);
    const data = await response.json();
  ...
  test("Handle invalid IP gracefully", async ({ request }) => {
    const response = await request.get(`${ENDPOINT}?ip=${testIPs.invalid}`);
  ...
  test("Check response time performance", async ({ request }) => {
    ...
    await request.get(`${ENDPOINT}?ip=${testIPs.valid}`);
    const duration = performance.now() - start;
    expect(duration).toBeLessThan(2000);
  });

  test("Check for response with private IP", async ({ request }) => {
    const response = await request.get(`${ENDPOINT}?ip=${testIPs.private}`);
    expect(response.status()).toBe(200);
    const data = await response.json();
  ...
  test("Check response with non-existent IP", async ({ request }) => {
    const response = await request.get(`${ENDPOINT}?ip=${testIPs.nonExistent}`);
    expect(response.status()).toBe(200);
    const data = await response.json();
  ...
  test("Check response with IPv6 address", async ({ request }) => {
    const response = await request.get(`${ENDPOINT}?ip=${testIPs.ipv6}`);
    expect(response.status()).toBe(200);
    const data = await response.json();
  ...
  test("Verify geo accuracy for another region", async ({ request }) => {
    const response = await request.get(`${ENDPOINT}?ip=${testIPs.googleDNS}`);
    expect(response.status()).toBe(200);
    const data = await response.json();
